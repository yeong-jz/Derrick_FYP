...
if (isset($_POST['submit'])){
  ...
  $query = "INSERT INTO deliver (id,username,price,description, title) VALUES ('".$id."','".$_SESSION['username']."','".$_POST['price']."','".$_POST['description']."','".$_POST['title']."')";
  mysqli_query($con,$query);
  $image = $_FILES['image']['tmp_name'];
  $target = "C:/wamp64/www/FYP2/tuitionimg/".basename($_FILES['image']['tmp_name']);
  move_uploaded_file($image,$target);
  $picture_insert = "UPDATE deliver SET picture='".basename($_FILES['image']['tmp_name'])."' WHERE id='".$id."'";
  mysqli_query($con, $picture_insert);
}

if(isset($_POST['confirmdeal'])){
	$insertquery="INSERT INTO deliver_deal (id,buyer,status) VALUES ('".$_POST['dealid']."','".$_SESSION['username']."','Unconfirmed')";
	mysqli_query($con,$insertquery);
}

$retrieve="SELECT accounts_info.Username, accounts_info.FullName, accounts_info.Image, deliver.id, deliver.title, deliver.price, deliver.picture FROM accounts_info INNER JOIN deliver ON accounts_info.Username=deliver.username";
...
while($row=mysqli_fetch_array($result)){
  ...
  echo "
          ...
          <div class='col-sm-3 col-xs-6'style='margin-bottom: 20px;'>
					<div class='row'>
						<div class='col-xs-3'>
							<img style='width: 35px; height: 35px; border-radius: 50%; margin-right: 60px;' onclick=\"viewprofile('$tusername')\" data-toggle='modal' data-target='#myModal4' src='/FYP2/accounts_photo/".$row['Image']."'>
						</div>
						<div class='col-xs-8'>
							<h5>".$row['FullName']."</h5>
						</div>
...
<div id="myModal4" class="modal fade" role="dialog">
 ...
 <div class="modal-content">
	 <div class="modal-header">
	<button type="button" class="close" data-dismiss="modal">&times;</button>
	<h4 class="modal-title">Provider Profile.</h4>
	 </div>
	 <div class="modal-body">
			 <p id="viewprofile"></p>
 	 </div>
...
function description(id){
 ...
 xmlhttp.open("GET","getDescription_deliver.php?id="+id,true);
 xmlhttp.send();
}
...
function viewprofile(x){
	...
	xmlhttp.open("GET","getProfile.php?username="+x,true);
	xmlhttp.send();
}
</script>
</html>
