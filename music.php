...
if (isset($_POST['submit'])){
  ...
  $query = "INSERT INTO music (id,username,price,description, title) VALUES ('".$id."','".$_SESSION['username']."','".$_POST['price']."','".$_POST['description']."','".$_POST['title']."')";
  mysqli_query($con,$query);
  $image = $_FILES['image']['tmp_name'];
  $target = "C:/wamp64/www/FYP2/tuitionimg/".basename($_FILES['image']['tmp_name']);
  move_uploaded_file($image,$target);
  $picture_insert = "UPDATE music SET picture='".basename($_FILES['image']['tmp_name'])."' WHERE id='".$id."'";
  mysqli_query($con, $picture_insert);
}

if(isset($_POST['confirmdeal'])){
	$insertquery="INSERT INTO music_deal (id,buyer,status) VALUES ('".$_POST['dealid']."','".$_SESSION['username']."','Unconfirmed')";
	mysqli_query($con,$insertquery);
}

$retrieve="SELECT accounts_info.Username, accounts_info.FullName, accounts_info.Image, music.id, music.title, music.price, music.picture FROM accounts_info INNER JOIN music ON accounts_info.Username=music.username";
...
function description(id){
 ...
 xmlhttp.open("GET","getDescription_music.php?id="+id,true);
 xmlhttp.send();
}
